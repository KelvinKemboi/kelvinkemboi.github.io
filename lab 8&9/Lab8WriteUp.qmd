...
```{r}
#| label: fig-box-binom
#| fig-cap: "PMF and CDF plots for Binomial Distribution."
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
...
```{r}
#| label: fig-box-negative
#| fig-cap: "PMF and CDF plots for Negative Distribution."
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
...
```{r}
#| label: fig-box-beta
#| fig-cap: "PMF and CDF plots for Beta -Binom Distribution."
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
...
```{r}
#| label: fig-box-prob
#| fig-cap: "PMF and CDF plots for P (X=0.70)."
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
...
```{r}
#| label: fig-box-p5
#| fig-cap: "PMF and CDF plots for P(X>=50)."
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
...
```{r}
#| label: fig-box-perc
#| fig-cap: "PMF and CDF plots for 90th Percentile."
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
...
inv.CDF.plot <- ggplot(data = gg.dat) +
  ...
  geom_point(data = tibble(CDF = 0.90, x = inv), aes(x = CDF, y = x), color = "darkorchid2") +
  theme_bw() +
...
```{r}
#| label: fig-box-bin
#| fig-cap: "PMF and CDF plots for Binom."
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
...
daltbinom <- function(x, size, p, t){
  i<- 0:size
  c<- sum(choose(size, i)*p^i*(1-p)^(size-i)*t^(i*(size-i))) # summation for formula
  ifelse(x >=0 & x<=size, choose(size, x)*p^x*(1-p)^(size-x)*t^(x*(size-x))/c, 0)
}
...
# created a tibble with two columns, PMF and CDF
# for 0.5
gg.data1 <- tibble(x = 0:3) |>
  mutate(
    PMF = daltbinom(x, size = 3, 0.70, 0.5),
    CDF = paltbinom(x, size = 3, 0.70, 0.5)
  )
p.win1<- 1 - paltbinom(1, size = 3, p = 0.70, t = 0.5)
(p.win1)

## for 1.0
gg.data2 <- tibble(x = 0:3) |>
  mutate(PMF= daltbinom(x, size = 3, 0.70, 1),
         CDF= paltbinom(x, size = 3, 0.70, 1)
        )
p.win2<- 1 - paltbinom(1, size = 3, p = 0.70, t = 1)
(p.win2)

## for 1.5
gg.data3 <- tibble(x = 0:3) |>
  mutate(
         PMF= daltbinom(x, size = 3, 0.70, 1.5),
         CDF= paltbinom(x, size = 3, 0.70, 1.5)
       )
p.win3 <- 1 - paltbinom(1, size = 3, p = 0.70, t = 1.5)
(p.win3)

# PMF plot for theta=0.5
PMF.plot1 <- ggplot(gg.data1) +
  geom_linerange(aes(x = x, ymin = 0, ymax = PMF)) +
  geom_hline(yintercept = 0) +
  theme_bw() +
  ylab("P(A wins) θ = 0.5")

# PMF plot for theta=1.0
PMF.plot2 <- ggplot(gg.data2) +
  geom_linerange(aes(x = x, ymin = 0, ymax = PMF)) +
  geom_hline(yintercept = 0) +
  theme_bw() +
  ylab("P(A wins) θ = 1")

# PMF plot for theta=1.5
PMF.plot3 <- ggplot(gg.data3) +
  geom_linerange(aes(x = x, ymin = 0, ymax = PMF)) +
  geom_hline(yintercept = 0) +
  theme_bw() +
  ylab("P(A wins) θ = 1.5")
