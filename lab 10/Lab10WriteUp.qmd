...
```{r}
#| label: fig-pmf-theta
#| fig-cap: "Probability that Player A wins as a function of θ under Altham’s Multiplicative Binomial Distribution (n = 11, p = 0.5)."
#| fig-width: 6
#| fig-height: 4
#| size: scriptsize
...
```{r}
#| label: fig-pmf-facets
#| fig-cap: "PMFs of Altham’s Multiplicative Binomial Distribution for θ = 0.5, 1, and 1.5 (n = 11, p = 0.5), shown in a single column for comparison."
#| fig-width: 5
#| fig-height: 8
#| size: scriptsize
...
gg.long<- gg.dat |> 
  ...
  mutate(theta= factor(theta, levels= c("PMF05", "PMF1", "PMF15"), labels= c("θ = 0.5", "θ = 1", "θ = 1.5")))
(gg.long)
# plot
...
```{r}
#| label: fig-match-dist
#| fig-cap: "Distribution of match outcomes by winner and loser legs."
#| fig-width: 6
#| fig-height: 4
#| fig-align: "center"
#| size: scriptsize
...
# plot summary
ggplot(summary.dat, aes(x = factor(WinnerLegs), y = Proportion, fill = factor(LoserLegs))) +
...
```{r}
#| label: fig-beta-player-dists
#| fig-cap: "Estimated Beta distributions for each player based on MLEs."
#| fig-width: 8
#| fig-height: 6
#| fig-align: "center"
#| size: scriptsize
...
l.likely.bb <- function(par, x, size, neg = FALSE){
  # transformations
  alpha<- exp(par[1])
  beta<- exp(par[2])
  theta<- exp(par[3])

  # compute log-likelihood match by match (size can vary per match)
  ll<- sum(mapply(function(xi, si){
    # compute probs for 0:si once per match
    probs <- daltbbinom(0:si, size=si, alpha=alpha, beta=beta, theta=theta)
    log(probs[xi + 1])
  }, x, size))
...
for(i in 1:length(players)){
  ...
  # legs won and total legs per match
  legs_won<- as.numeric(if_else(matches$Winner == player,
                                   matches$WinnerLegs, matches$LoserLegs))
  total_legs<- as.numeric(matches$WinnerLegs) + as.numeric(matches$LoserLegs)

  # MLE with optim
  res<- optim(par=c(0, 0, 0), fn=l.likely.bb,
               x=legs_won, size=total_legs, neg=TRUE)
  ...
  beta_sd[i]<- sqrt((alpha_estims[i] * beta_estims[i])/((alpha_estims[i] + beta_estims[i])^2*(alpha_estims[i] + beta_estims[i]+1)))
}
...
beta.plot.dat<- tibble()

for(i in 1:length(players)){
  beta.plot.dat<- bind_rows(beta.plot.dat, tibble(
    Player = players[i],
    p = p_vals,
    density = dbeta(p_vals, alpha_estims[i], beta_estims[i])
  ))
}

# plot
ggplot(beta.plot.dat, aes(x = p, y = density)) +
  geom_line() +
  facet_wrap(~ Player, scales = "free_y") +
  theme_bw() +
  labs(
    title = "Estimated Beta Distributions by Player",
    x     = "Leg win probability",
    y     = "Density"
  )
```
