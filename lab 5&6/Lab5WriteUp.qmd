...
**Solution:**
Installed packages `tidyverse` and `patchwork`
Downloaded the souce data and moved the file to my working directory (in the lab folder)
Made the `xlxs` file rectangular for effecient use in `R` by stacking duplicate columns.
Converted it to a csv file to make it easier to read and work with. 
Loaded the file using `.csv`

```{r}
#| size: scriptsize
biomed.dat<-read.csv("BiomedData.csv")
```
...
Used the `seperate()` function to split the data uder `sample` into `SampleType`, `CellType`, `Treatment` columns. Saved it in `dat.kitsberg.cleaned`
```{r}
#| size: scriptsize
dat.kitsberg.cleaned<- biomed.dat |>
...
Used `filter(...)` to get only the treatment group with `12hpi`
```{r}
#| size: scriptsize
...
Used the `mutate(...)` function to change the `MDM` types to `mac` types. Used `if_else` conditional statements for this.
```{r}
#| size: scriptsize
...
Using `filter(...)` function to seperate only monocytes and macrophages.
```{r}
#| size: scriptsize
...
Used `mutate` to replace shorthand notations with the neat types.
```{r}
#| size: scriptsize

# replacing labels using mutate
dat.kitsberg.cleaned <- dat.kitsberg.cleaned |>
  ...
  mutate(Treatment = if_else(Treatment=="12hpi", "12 Hours Post-Infection", "Uninfected"))
```
...
Combined my answers into one block of code for the next section
```{r}
#| size: scriptsize

# full cleaning code
dat.kitsberg.cleaned<- biomed.dat |>
  separate(col="sample", into=c("SampleType", "CellType", "Treatment"), sep="_") |> # seperate samples into different tupes
  filter(Treatment=="12hpi")|> # filter by 12hpi
  mutate(CellType = if_else(CellType=="MDM", "mac", CellType))|> 
  filter(CellType=="mono" | CellType=="mac")|>
  mutate(SampleType = if_else(SampleType=="primary", "Healthy Donor", "THP-1")) |> # remaming sample types
  mutate(CellType = if_else(CellType=="mono", "Monocytes", "Macrophages")) |> # renaming cell types
  mutate(Treatment = if_else(Treatment=="12hpi", "12 Hours Post-Infection", "Uninfected")) # renaming treatments

head(dat.kitsberg.cleaned)
```
...
Used the function `geom_violin` of `ggplot` to plot a violin graph visualisation of the HCMV genomes detected in the nucleus and cytoplasm of infected monocytes compared to macrophages.
```{r}
#| size: scriptsize
glimpse(dat.kitsberg.cleaned)
ggplot(data= dat.kitsberg.cleaned) +
  geom_violin(aes(x= CellType, y= viruses.within.nucleus), width = 1) +
  theme_bw() +
  facet_wrap(~SampleType) + 
  labs(
    x= "Cell Types",
    caption= "Fig 3b: HCMV genomes detected in the nucleus and cytoplasm of infected monocytes compared to macrophages.",
    y = "Viral genomes detected in nucleus"
  )
```
...
Used `group_by(...)` and `summarize(...)` to create a summary of the relevant numerical summaries in `dat.kitsberg.cleaned` data.

```{r}
#| size: scriptsize

# numerical summary for the data grouped by cell type
dat.kitsberg.summary <- dat.kitsberg.cleaned |>
  group_by(CellType) |> # get celltype summaries
  summarize(
    n = n(), # number
    Missing = sum(is.na(viruses.within.nucleus)), # missing values
...
Used the `knitr::kable(...)` function and adding `#| echo: false` to
print the table nicely is hidden by setting `echo: false` in the header
```{r}
#| label: tbl-sampletype-viruses
#| tbl-cap: "Viral genomes detected in nucleus in Sample Types."
#| tbl-align: "center"
#| echo: false
dat.kitsberg.summary |>
  knitr::kable(digits = 2) # display in a nice table
```
...
Loaded the downloaded data from the lab folder using `read_csv(...)`
```{r}
#| size: scriptsize
ship.data<-read.csv("ShipValuation-Bal25.csv")
```
...
Used the `mutate(...)` function to create a new column `price` containing ship prices in millions of dollars. Saved the object as `dat.bal.cleaned` 

```{r}
#| size: scriptsize
dat.bal.cleaned<-ship.data |>
  mutate(Price= (10^SPLog)/1000000) # add price column
```
...
Used `case_when` and `mutate(...)` functions to find the the origin countries marked with 1 the updated `dat.bal.cleaned`.

```{r}
#| size: scriptsize

# new origincountry column with renamed countries
...
Used `mutate(...)` and `case_when(...)` to create a new column `Manufacturer` containing the labels for manufacturers

```{r}
#| size: scriptsize

# new manufacturer column with renamed manufaturer labels
dat.bal.cleaned <- dat.bal.cleaned |>
...
Used `mutate(...)` and `case_when(...)` to create a new column `PI.club` containing labels for the P&I club
```{r}
#| size: scriptsize

# new PIclub column with renamed labels
dat.bal.cleaned <- dat.bal.cleaned |>
...
Used `mutate(...)` and `case_when(...)` to create a new column `Class` containing labels for class
```{r}
#| size: scriptsize

# new class column with renamed class labels
dat.bal.cleaned <- dat.bal.cleaned |>
...
Used `mutate(...)` and `case_when(...)` to create a new column `FlagState` containing labels for flag state
```{r}
#| size: scriptsize

# new flagstate columns with renamed flagstate labels
dat.bal.cleaned <- dat.bal.cleaned |>
...
# combined cleaning code
dat.bal.cleaned<-ship.data |>
  mutate(Price= (10^SPLog)/1000000) |> # price
  mutate(OriginCountry= case_when( # origincountry column
    Japan==1 ~ "Japan",
    China..People.s.Republic.Of ==1 ~ "China",
    Philippines ==1 ~ "Philippines",
    Korea..South == 1 ~ "South Korea",
  ...
  mutate(Manufacturer= case_when( # manufacturer column
    Caterpillar==1 ~ "Caterpillar",
    MAN.B.W ==1 ~ "MAN.B.W",
    Wartsila ==1 ~ "Wartsila",
    Mitsubishi == 1 ~ "Mitsubishi",
  ...
  mutate(PI.club= case_when( # PIClub column
    Japan.P.I.Club==1 ~ "Japan P.I Club",
    Gard ==1 ~ "Gard",
    West.of.England ==1 ~ "West of England",
    North.of.England == 1 ~ "North of England",
  ...
  mutate(Class= case_when( # class column
    Nippon.Kaiji.Kyokai..IACS.==1 ~ "Nippon Kaiji Kyokai (IACS)",
    Lloyd.s.Register ==1 ~ "Lloyd's Register",
    Bureau.Veritas..IACS. ==1 ~ "Bureau Veritas (IACS)",
    Det.Norske.Veritas..IACS. == 1 ~ "Det Norske Veritas (IACS)",
  ...
  mutate(FlagState= case_when( # flagstate column
    Panama==1 ~ "Panama",
    Marshall.Islands ==1 ~ "Marshall Islands",
    Liberia ==1 ~ "Liberia",
    Malta == 1 ~ "Malta",
...
Used the function `geom_violin` of `ggplot` to plot a violin graph visualisation of a ships origin country in relation to the prices.
...
ggplot(data= dat.bal.cleaned) +
  geom_violin(aes(x= OriginCountry, y= Price), width = 1) +
  geom_boxplot(aes(x= OriginCountry,y= Price),width=0.3) +
  theme_bw() +
  labs(
    x= "Origin countries",
    title= "Violin graph visualisation of a ships origin country in relation to the prices.",
    y = "Ship prices",
  )
```

Used `group_by(...)` and `summarize(...)` to create a summary of the relevant numerical summaries in `dat.bal.cleaned` data.

```{r}
#| size: scriptsize
dat.bal.summary <- dat.bal.cleaned |>
...
#| tbl-cap: "Ships origin country in relation to the prices."
#| tbl-align: "center"
#| echo: false
dat.bal.summary |>
...
Used the function `geom_point` of `ggplot` to plot a scatter plot visualisation of a ships' Yas in relation to the prices.

```{r}
#| size: scriptsize
ggplot(data= dat.bal.cleaned) +
  geom_point(aes(x= Yas, y= Price), width = 1) +
  theme_bw() +
  labs(
    x= "Yas",
    title= "Scatter plot visualisation of a ships' Yas in relation to the prices.",
    y = "Ship prices",
  )
```
...
```{r}
#| label: tbl-Yas
#| tbl-cap: "Ships Yas in relation to the prices"
#| tbl-align: "center"
#| echo: false
dat.bal.summary |>
  knitr::kable(digits = 2)
